...
async function createTables(db) {
  await db.exec(
    `CREATE TABLE movies (id INTEGER PRIMARY KEY, title TEXT, durationMins INTEGER)`
  );
  await db.exec(
    `CREATE TABLE cinemas (id INTEGER PRIMARY KEY, location TEXT, num_of_screens INTEGER)`
  );
  await db.exec(
    `CREATE TABLE screenings (id INTEGER PRIMARY KEY, start_time INTEGER, movie_id INTEGER, cinema_id, FOREIGN KEY(movie_id) REFERENCES movies(id), FOREIGN KEY(cinema_id) REFERENCES cinemas(id))`
  );
}
